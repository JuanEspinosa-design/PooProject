...
void Ecosistema::mostrarMapa() const {
    ...
    for (int i = 0; i < mapa->getFilas(); ++i) {
        ...
        for (int j = 0; j < mapa->getColumnas(); ++j) {
            ...
            bool hayGlorbo = false, hayJirafa = false, hayPlanta = false;

            for (const auto& criatura : celda.getcriaturas()) {
                if (dynamic_cast<glorbo*>(criatura.get())) hayGlorbo = true;
                else if (dynamic_cast<jirafa*>(criatura.get())) hayJirafa = true;
                else if (dynamic_cast<Planta*>(criatura.get())) hayPlanta = true;
            }

            if (hayGlorbo) contenido += "G";
            if (hayJirafa) contenido += "J";
            if (hayPlanta) contenido += "P";
